    ...
    try {
      ...
      let layersInfo = _.filter(
        ...
        e => 
          _.find(
            e.tags,
            tag => tag.key === "layer_sequence_index"
          ) !== undefined
      )
    ...
    return(
      <React.Fragment>
        ...
        <h1 style={{ textAlign: "center" }}>
          <a href={this.props.traceURL}> Trace </a>
        </h1>
        <Iframe position="relative" url={this.props.traceURL} width="100%" height="500px" />

        {this.state.graphData !== null &&
        <div style={{marginTop: "40px"}}>
          <h1 style={{ textAlign: "center" }}>Layer durations Informations</h1>
          <Chart padding="auto" height={this.state.graphData.length * 20} data={this.state.graphData} forceFit>
            <Coord transpose />
            <Axis
              name="layer"
              labelOffset={10}
